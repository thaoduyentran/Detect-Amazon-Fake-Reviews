...
#### loading & reformatting data ####

df2008 <- read.csv("2008_data.csv", sep = "|")

df2018 <- read.csv("2018_data.csv", sep = "|")
...
df2008$overall <- as.factor(df2008$overall)
df2018$overall <- as.factor(df2018$overall)

df2008$reviewerID <- as.factor(df2008$reviewerID)
df2018$reviewerID <- as.factor(df2018$reviewerID)
...
sample2008 <- sample(1:nrow(subset2008), 2000, replace=F, prob = NULL)
sample2018 <- sample(1:nrow(subset2018), 2000, replace=F, prob = NULL)

df2008 <- subset2008[sample2008,]
df2018 <- subset2018[sample2018,]


summary(df2008)
summary(df2018)

#### make df with non unique review text ####

notunique2008 <- subset2008

notunique2008 <- notunique2008[notunique2008$reviewText %in% 
                           notunique2008$reviewText[duplicated(notunique2008$reviewText)],]

notunique2018 <- subset2018
notunique2018 <- notunique2018[notunique2018$reviewText %in% 
                                 notunique2018$reviewText[duplicated(notunique2018$reviewText)],]

#replace links and video
notunique2018$reviewText <- str_replace(notunique2018$reviewText, "\\<a+[:print:]+\\</a\\>",
                                     " *INCLUDES_LINK* ")
notunique2018$reviewText <- str_replace(notunique2018$reviewText, "\\&nbsp\\;","")
notunique2018$reviewText <- str_replace(notunique2018$reviewText, "\\<div+[:print:]+\\>",
                                     " *INCLUDES_VIDEO* ")



#### ISOLATE 5* AND 1* REVIEWS ####

unique2008_1s <- unique2008 %>% filter(overall=="1") %>% arrange(reviewerID)
unique2008_5s <- unique2008 %>% filter(overall=="5") %>% arrange(reviewerID)

unique2018_1s <- unique2018 %>% filter(overall=="1") %>% arrange(reviewerID)
unique2018_5s <- unique2018 %>% filter(overall=="5") %>% arrange(reviewerID)


notunique2008_1s <- notunique2008 %>% filter(overall=="1") %>% arrange(reviewerID)
notunique2008_5s <- notunique2008 %>% filter(overall=="5") %>% arrange(reviewerID)

notunique2018_1s <- notunique2018 %>% filter(overall=="1") %>% arrange(reviewerID)
notunique2018_5s <- notunique2018 %>% filter(overall=="5") %>% arrange(reviewerID)
...
#for unique 2008 and 2018  1* reviews
words_unique2008_1s <- unique2008_1s %>%
  ungroup() %>%
  select(c("overall", "verified", "reviewTime", "reviewerID", "asin", "reviewText")) %>%
  unnest_tokens(word, reviewText) %>%
  filter(!word %in% stop_words$word, str_detect(word, "^[a-z']+$"))

words_unique2018_1s <- unique2018_1s %>%
  ungroup() %>%
  select(c("overall", "verified", "reviewTime", "reviewerID", "asin", "reviewText")) %>%
  unnest_tokens(word, reviewText) %>%
  filter(!word %in% stop_words$word, str_detect(word, "^[a-z']+$"))
...
#for unique 2008 and 2018 5* reviews
words_unique2008_5s <- unique2008_5s %>%
  ungroup() %>%
  select(c("overall", "verified", "reviewTime", "reviewerID", "asin", "reviewText")) %>%
  unnest_tokens(word, reviewText) %>%
  filter(!word %in% stop_words$word, str_detect(word, "^[a-z']+$"))

words_unique2018_5s <- unique2018_5s %>%
  ungroup() %>%
  select(c("overall", "verified", "reviewTime", "reviewerID", "asin", "reviewText")) %>%
  unnest_tokens(word, reviewText) %>%
  filter(!word %in% stop_words$word, str_detect(word, "^[a-z']+$"))
...
# nrc for 2008 
nrc_unique2008_1s <- words_unique2008_1s %>% inner_join(nrc, by="word") %>%
                     mutate(sent_emot = if_else(
                            sentiment %in% c('negative','positive'),'sentiment','emotion'))
nrc_unique2008_5s <- words_unique2008_5s %>% inner_join(nrc, by="word") %>%
                     mutate(sent_emot = if_else(
                     sentiment %in% c('negative','positive'),'sentiment','emotion'))

#nrc for 2018
nrc_unique2018_1s <- words_unique2018_1s %>% inner_join(nrc, by="word") %>%
                     mutate(sent_emot = if_else(
                     sentiment %in% c('negative','positive'),'sentiment','emotion'))
nrc_unique2018_5s <- words_unique2018_5s %>% inner_join(nrc, by="word") %>%
                     mutate(sent_emot = if_else(
                     sentiment %in% c('negative','positive'),'sentiment','emotion')) 

#checking the counts for # of sentiments and whether it's a sentiment or emotion
words_unique2018_5s %>% inner_join(nrc, by="word") %>% 
  count(sentiment) %>% arrange(desc(n))
...
#overall rating ~ verified reviews 2008
unique2008 %>% ggplot(aes(x=overall)) + 
  geom_bar(aes(fill=verified), position="dodge") +
  labs(title="Number of Verified Reviews in 2008 (June-Sept)",
       x="Product Rating (Number of Stars)", y="Number of Review",
...
#overall rating ~ verified reviews 2008
unique2018 %>% ggplot(aes(x=overall)) + 
  geom_bar(aes(fill=verified), position="dodge") +
  labs(title="Number of Verified Reviews in 2018 (June-Sept)",
       x="Product Rating (Number of Stars)", y="Number of Review",
       fill="Verified Review") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
...
# 2008 & 2018 1* reviews
nrc_unique2008_1s %>% ggplot(aes(x=str_to_title(sentiment))) + 
  geom_bar(aes(fill=str_to_title(sent_emot))) +
  facet_wrap(~verified, labeller=label_both,) +
  labs(title="Sentiment Analysis for 2008's 1-star Reviews (June-Sept)",
         y="Count", x="Sentiment/Emotion", fill="Color") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
  geom_text(stat="count", aes(label=..count..,group=sentiment), cex=2.5, 
            vjust=-0.5, hjust=0.5, position=position_dodge(width=1))+
...
#### THIS IS WHERE YOU LEFT OFF. THE BOTTOM GRAPH IS THE SAME AS THE TOP
#THE ONLY DIFFERENCE IS FACET WRAP VS. DODGE BAR GRAPH
#YOU DID A GREAT JOB YOU MOTHERFUCKER HAHAHAHAH
nrc_unique2008_1s %>% ggplot(aes(x=str_to_title(sentiment)),position="dodge") + 
  geom_bar(aes(fill=str_to_title(verified)),position = "dodge")  +
  labs(title="Sentiment Analysis for 2008's 1-star Reviews (June-Sept)",
       y="Count", x="Sentiment/Emotion", fill="Verified") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
  geom_text(stat="count", aes(label=..count..,group=verified), cex=2.5, 
            vjust=-0.5, hjust=0.5, position=position_dodge(width=1))+
...
nrc_unique2018_1s %>% ggplot(aes(x=str_to_title(sentiment))) + 
  geom_bar(aes(fill=str_to_title(sent_emot))) +
  labs(title="Sentiment Analysis for 2018's 1-star Reviews (June-Sept)",
       y="Count", x="Sentiment/Emotion", fill="Color") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
  geom_text(stat="count", aes(label=..count..,group=sentiment), cex=2.5, 
...
# 2008 & 2018 5* reviews
nrc_unique2008_5s %>% ggplot(aes(x=str_to_title(sentiment))) + 
  geom_bar(aes(fill=str_to_title(sent_emot))) +
  labs(title="Sentiment Analysis for 2008's 5-star Reviews (June-Sept)",
       y="Count", x="Sentiment/Emotion", fill="Color") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
  geom_text(stat="count", aes(label=..count..,group=sentiment), cex=2.5, 
...
nrc_unique2018_5s %>% ggplot(aes(x=str_to_title(sentiment))) + 
  geom_bar(aes(fill=str_to_title(sent_emot))) +
  labs(title="Sentiment Analysis for 2018's 5-star Reviews (June-Sept)",
       y="Count", x="Sentiment/Emotion", fill="Color") +
  theme_classic()+
  scale_fill_manual(values=c("#FF9900","#146EB4"))+
  geom_text(stat="count", aes(label=..count..,group=sentiment), cex=2.5, 
...
#cloud for 2008 and 2018 5*

#verified=TRUE
summary_unique2008_5s <- words_unique2008_5s %>% ungroup() %>% 
  select(1:6) %>% filter(verified=="True") %>%
  inner_join(afinn, by="word") %>% group_by(word) %>% 
  summarise(word=word, value=value, word_count=n()) %>%
  distinct() %>% arrange(desc(word_count))

wordcloud(words = summary_unique2008_5s$word, freq = summary_unique2008_5s$word_count, 
          max.words=100, colors=rep(amzn_cols), scale = c(3,0.5), random.order = F)


summary_unique2018_5s <- words_unique2018_5s %>% ungroup() %>% 
  select(1:6) %>% filter(verified=="True") %>%
  inner_join(afinn, by="word") %>% group_by(word) %>% 
  summarise(word=word, value=value, word_count=n()) %>%
  distinct() %>% arrange(desc(word_count))

wordcloud(words = summary_unique2018_5s$word, freq = summary_unique2018_5s$word_count, 
          max.words=100, colors=rep(amzn_cols), scale = c(3,0.5), random.order = F)

#verified=FALSE
summary_unique2008_5s <- words_unique2008_5s %>% ungroup() %>% 
  select(1:6) %>% filter(verified=="False") %>%
  inner_join(afinn, by="word") %>% group_by(word) %>% 
  summarise(word=word, value=value, word_count=n()) %>%
  distinct() %>% arrange(desc(word_count))
...
misspelled2 <- misspelled2 %>%
  add_column(verified = fivestar2018$verified,
             .after = "n_misspelled") 
# check_spelling
...
misspel = read.csv('~/Downloads/misspell.csv')
View(misspel)
plot(x=misspel$misspell_prop, y =misspel$rating)
qplot